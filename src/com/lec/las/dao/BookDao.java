...
public class BookDao {
	...
	public int insertBook(BookDto dto) {
		...
		String sql = "INSERT INTO BOOK (bID, bTITLE, bAUTHOR, bIMAGE1, bIMAGE2, bCONTENT) " + 
	...
	public ArrayList<BookDto> listBook(int startRow, int endRow){
		...
		try {
			...
			while(rs.next()) {
				...
				books.add(new BookDto(bid, btitle, bauthor, bimage1, bimage2, bcontent, brdate));
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
	...
	public BookDto getBook(int bid) {
		...
		try {
			...
			if(rs.next()) {
				...
				String bcontent  = rs.getString("bcontent");
				Timestamp brdate = rs.getTimestamp("brdate");
				book = new BookDto(bid, btitle, bauthor, bimage1, bimage2, bcontent, brdate);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
	...
	// 5. 도서삭제
	public int withdrawalBook(int bid) {
		int result = FAIL;
		Connection        conn  = null;
		PreparedStatement pstmt = null;
